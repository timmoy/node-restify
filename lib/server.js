...
Server.prototype.debugInfo = function debugInfo() {
    ...
    if (!self._debugInfo) {
        ...
        for (routeName in router.mounts) {

            if (router.mounts.hasOwnProperty(routeName)) {

                var mountedRoute = router.mounts[routeName];
                var routeRegex = mountedRoute.path;

                routeInfo.push({
                    name: mountedRoute.name,
                    method: mountedRoute.method,
                    ...
                    handlers: self.routes[mountedRoute.name]
                });
            }
        }
